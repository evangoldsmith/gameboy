...
// sequencer, and resamples their combined output down to a normal audio rate.
// The frontend drains samples() once a frame; the core has no SDL dependency.
class APU {
public:
...
private:
    ...
    // Wave RAM is stored here so writes are not lost, but nothing plays it
    // until channel 3 exists.
